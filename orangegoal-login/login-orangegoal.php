	...
	class Orangegoal_Login_Plugin {
		...
		public static function plugin_activated() {
			$page_definitions = array(
				'member-login' => array(
					'title' => __('Please, sign in', 'login-orangegoal'),
					'content' => '[login-orangegoal-form]'
					),
				'member-account' => array(
					'title' => __('Welcome to orangegoal','login-orangegoal'),
		...
		public function render_login_form($attributes,$content=null) {
			...
			if (is_user_logged_in()){
				return __('You are already signed in.','login-orangegoal');
			}
			//error messages
			$errors = array();
		...
		private function get_error_message($error_code){
			switch($error_code){
				...
				case 'email_exists':
				return __('An account exists with this email address.','login-orangegoal');
				case 'closed':
				return __('Registering new users is currently not allowed.','login-orangegoal');
				case 'empty_username':
				return __('Please, tell us your e-mail address or username','login-orangegoal');
				case 'empty_password':
				return __('You need your password to login.','login-orangegoal');
				case 'invalid_username':
				return __("We dont't have any users with that e-mail address. Maybe you aren't registered with us already.",'login-orangegoal');
		...
		public function redirect_to_dashboard() {
			if ('GET' == $_SERVER['REQUEST_METHOD']) {
				if(is_user_logged_in()){
					...
					wp_redirect('register-to-orangegoal');
				}
				exit;
			}
		}
		...
		public function do_register_user() {
			if('POST' == $_SERVER['REQUEST_METHOD']){
				$redirect_url = home_url('register-to-orangegoal');
